...
public class MovieAnalyzer {
    ...
    public static class Movie{
        ...
        public Movie(String series_Title, Integer released_Year, String certificate, Integer runtime,
                     String genre, Float IMDB_Rating, String overview, Integer meta_score,
    ...
    public List<String> searchMovies(String genre, float min_rating, int max_runtime){
        ...
        for (Movie datum : data) {
            if (datum.getGenre().contains(genre) && datum.getIMDB_Rating() >= min_rating && datum.getIMDB_Rating() <= 10 && datum.getRuntime() <= max_runtime && datum.getRuntime() > 0) {
                list.add(datum);
            }
        }
